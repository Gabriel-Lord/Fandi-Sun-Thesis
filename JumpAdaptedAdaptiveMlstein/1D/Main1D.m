...
for m=1:hmaxListLength % go through hmaxlist list
    ...
    WincKeep=zeros(M,parms.maxLength);
    ...
    for i=1:M  % run through all MC realisations
        ...
        for k=1:timesIncILen  % run through reference mesh points
            ...
            % main scheme
            Xref=schemeJAdptPMil(parms, Xref, Wref, dt, JumpSize);
            t=t+dt;
        end
        ...
        [XAMil, j, hfix]=schemeJAdptAMil(parms, XAMil, Winc, i, j, hfix);
    ...
    for i=1:M % run through MC realisations for all fixed-step methods
        ...
        WincRefTemp=nonzeros(WincKeep(i,:))';
        ...
        for k=1:timesIncDtLen % run through mesh points
            ...
            WincDt=sum(WincRefTemp(Marker+1 : hLocCeil-2));
            ...
            if jumpIndct == 0  % true if it is not landing at jump mesh point
                if  isempty(hLocCeil) ~=  true  % if found
                    ...
                    WincDt=sum(WincRefTemp(Marker+1 : end));
                end
            end
            ...
            XTMil=schemeJAdptTMil(parms, XTMil, WincDt, dt, JumpSize);
            XPMil=schemeJAdptPMil(parms, XPMil, WincDt, dt, JumpSize);
            XSSBM=schemeJAdptSSBM(parms, XSSBM, WincDt, dt, funcCubicDrift, JumpSize);
            XTSRK=schemeJAdptTSRK(parms, XTSRK, WincDt, dt, JumpSize);
            
            t=t+dt;
        end
        % adding sqaured error together for the sample mean later
        XerrPMil(m)=XerrPMil(m)+abs(XrefKeep(i)-XPMil)^2;
        XerrTMil(m)=XerrTMil(m)+abs(XrefKeep(i)-XTMil)^2;
        XerrSSBM(m)=XerrSSBM(m)+abs(XrefKeep(i)-XSSBM)^2;
        XerrTSRK(m)=XerrTSRK(m)+abs(XrefKeep(i)-XTSRK)^2;
...
for m=1:hmaxListLength % go through hmaxlist list
    ...
    for i=1:M
        ...
        tAMil=tic;
        XAMil = Xzero;
        XAMil=schemeJAdptAMilCPU(parms, i, XAMil);
