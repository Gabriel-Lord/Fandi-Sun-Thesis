function [outcome, j, hfix]=schemeJAdptAMil2D(parms, StartValue, Winc, i, j, hfix)
...
% % 'Winc' -- (2-by-n vector) Wiener increment from the reference solution
...
while abs(t-parms.T)>parms.Tol && t<=parms.T
    ...
    WincAMil=sum(WincTemp(:, Marker+1 : hLocCeil-2),2);
    ...
    if jumpIndct == 0  % true if it is not landing at jump mesh point
        if  isempty(hLocCeil) ~=  true  % if found
            ...
            WincAMil=sum(WincTemp(:,Marker+1 : end),2);
        end
    end
    ...
    if h <= parms.hmin % backstop case
        % we use JA-PMil with timestep 'h' for the backstop
