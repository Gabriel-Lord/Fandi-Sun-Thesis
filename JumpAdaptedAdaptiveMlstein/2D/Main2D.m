...
%     2. diffusion with diagonal, commutative in 'DIFF.m'
...
for m=1:hmaxListLength % go through hmaxlist list
    ...
    for i=1:M  % run through all MC realisations
        ...
        Winc=randn(rank,timesIncILen).*sqrt(timesIncI);
        % store reference Wiener increments
        WincKeep(rank*i-(rank-1):rank*i,1:size(Winc,2))=Winc;
        Xref=Xzero;  t=0;
    ...
    for i=1:M % run through MC realisations for all fixed-step methods
        ...
        JumpTimesTemp=[nonzeros(parms.JumpTimes(i,:))',T+Tol];
        zetaTemp=parms.zeta(rank*i-(rank-1):rank*i,:);
        WincRefTemp=WincKeep(rank*i-(rank-1):rank*i,:);
