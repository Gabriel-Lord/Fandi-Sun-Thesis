...
% boundsplus  = to expand the pos of the zoom box with [x_leftminus, x_rightplus, y_downminus, y_upplus]
 

function [p z] = zoomPlot(x,y,style,gca1,xbounds,pos,boundsplus,varargin)
...
x1 = (pos(1)-p.Position(1))/p.Position(3)*diff(p.XLim)+p.XLim(1)+boundsplus(1);
x2 = (pos(1)+pos(3)-p.Position(1))/p.Position(3)*diff(p.XLim)+(p.XLim(1))+boundsplus(2);
y1 = (pos(2)-p.Position(2))/p.Position(4)*diff(p.YLim)+p.YLim(1)+boundsplus(3);
y2 = ((pos(2)+pos(4)-p.Position(2))/p.Position(4))*diff(p.YLim)+p.YLim(1)+boundsplus(4);
...
rectangle('Position',[xbounds(1)+boundsplus(1), min(y(index))+boundsplus(3),...
    diff(xbounds)+boundsplus(2), max(y(index))-min(y(index))+boundsplus(4)]);
hold on
if any(vertex==1)
%     [xbounds(1) x1]
%      [max(y(index)) y2]
    plot([xbounds(1) x1], [min(y(index)) y2], 'k','LineWidth',1) % Line to vertex 1
end
if any(vertex==2)
%     [xbounds(2) x2]
%     [max(y(index)) y2]
    plot([xbounds(2) x2], [min(y(index)) y2], 'k','LineWidth',1) % Line to vertex 2
end
if any(vertex==3)
%     [xbounds(2) x2]
%     [min(y(index)) y1]
    plot([xbounds(2) x2], [min(y(index)) y1], 'k','LineWidth',1) % Line to vertex 4
end
if any(vertex==4)
%     [xbounds(1) x1]
%     [min(y(index)) y1]
    plot([xbounds(1) x1], [min(y(index)) y1], 'k','LineWidth',1) % Line to vertex 3
end
...
box on 
plot(x,y,style{1},'LineWidth',style{2},'MarkerSize',style{3})
axis([xbounds(1)+boundsplus(1), xbounds(2)+boundsplus(2),...
    min(y(index))+boundsplus(3) max(y(index))+boundsplus(4)]);
